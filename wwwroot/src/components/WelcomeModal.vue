...
const emit = defineEmits<{
  ...
  projectOpenFailed: [message: string]
}>()

const recentProjects = ref<RecentProject[]>([])
const isLoadingRecents = ref(false)
const openingRecentPath = ref<string | null>(null)
...
async function handleOpenRecent(projectPath: string) {
  ...
  emit('projectOpenFailed', projectImport.error.value || '项目打开失败，请检查 Project.tlor 和 Scripts/Main 目录。')
  recentProjects.value = await getRecentProjects()
}
          ...
          <div v-else class="wm-recent-list">
            ...
            >
              ...
              <span v-if="project.lastOpened" class="wm-recent-date">
                {{ openingRecentPath === project.path ? t('Startup.Loading') : new Date(project.lastOpened).toLocaleDateString() }}
              </span>
            </button>
          </div>
        </div>
