...
export const useEditorStore = defineStore('editor', () => {
  ...
  async function save() {
    const sceneId = nodeGraphStore.currentSceneId
      || currentFileName.value.replace(/\\/g, '/').split('/').pop()?.replace(/\.lor$/i, '')

    if (!sceneId || sceneId === 'Untitled') {
    ...
    try {
      ...
      currentFileName.value = `${sceneId}.lor`
